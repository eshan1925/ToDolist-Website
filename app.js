                ...
                if (err) {
                    ...
                    if (foundItems.length === 0) {
                        ...
                        User.find({ _id: presentUserId }, function (err, currentUser) {
                            if (!err) {
                                res.render("list", { listTitle: "Tasks", newListItems: foundItems, buttonName: "Work List🏢", secondButtonName: "Archieves ✅", redirectLocation: "/work/" + presentUserId, secondRedirectLocation: "/archieves/" + presentUserId, presentUser: presentUserId, userName: currentUser[0].username, adminButton: "Today📆", adminRedirectLocation: "/past/" + presentUserId });
                            }
                        })
                    }
                }
            ...
            Item.find({ id: presentUserId }, function (err, foundItems) {
                if (err) {
                    ...
                    if (foundItems.length === 0) {
                        ...
                        User.find({ _id: presentUserId }, function (err, currentUser) {
                            if (!err) {
                                res.render("list", { listTitle: "Tasks", newListItems: foundItems, buttonName: "Work List🏢", secondButtonName: "Archieves ✅", redirectLocation: "/work/" + presentUserId, secondRedirectLocation: "/archieves/" + presentUserId, presentUser: presentUserId, userName: currentUser[0].username });
                            }
                        })
                    }
                }
            ...
            Item.find({}, function (err, foundItems) {
                if (err) {
                    ...
                    if (foundItems.length === 0) {
                        ...
                        User.find({ _id: presentUserId }, function (err, currentUser) {
                            if (!err) {
                                res.render("list", { listTitle: "Archieves", newListItems: foundItems, buttonName: "Work List🏢", secondButtonName: "ToDos🧐", redirectLocation: "/work/" + presentUserId, secondRedirectLocation: "/list/" + presentUserId, presentUser: presentUserId, userName: currentUser[0].username,adminButton: "Today📆", adminRedirectLocation: "/past/" + presentUserId });
                            }
                        })
                    }
                }
...
app.get("/past/:userId", function (req, res) {
    if (req.isAuthenticated()) {
        const presentUserId = req.params.userId;
        if (presentUserId === "62b05e84ae52c59a3a68ac08") {
            var d = new Date();
            ...
            if (month < 9) {
                if (day < 9) {
                    var compDate = "0" + month + "/" + "0" + day + "/" + year;
                } else {
                    var compDate = "0" + month + "/" + day + "/" + year;
                }
            } else {
                if (day < 9) {
                    var compDate = month + "/" + "0" + day + "/" + year;
                } else {
                    var compDate = month + "/" + day + "/" + year;
                }
            }
            Item.find({ dateOfCompletion: compDate }, function (err, foundItems) {
                if (err) {
                    console.log(err.message);
                } else {
                    console.log("Successfully found Items!!!");
...
app.post("/delete/:userId", function (req, res) {
    ...
    if (listName === "Tasks" || listName === "Archieves") {
